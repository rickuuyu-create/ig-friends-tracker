...
不確定朋友的英文名怎麼讀時，不用再複製去翻譯網站。

- 首頁清單和朋友詳細頁的名字旁邊都有一個喇叭圖示，點一下就會讀出名字。
- 正在播放時再點一次即可停止；點另一位朋友的喇叭會直接改讀那一位。
- 讀音由瀏覽器內建的語音功能產生，名字不會傳送到任何伺服器。
- 名字是中文會用國語讀，其他情況用英文讀。
- 少數瀏覽器沒有內建語音功能，這時喇叭圖示不會出現。
...
第一次登入會自動顯示聚光式教學，介紹設定、搜尋、多標籤篩選、名字讀音、新增朋友、生日、頭像、永久 ID 和儲存流程。名單上未有朋友時，讀音那一步會自動略過。
